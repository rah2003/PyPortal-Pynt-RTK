...
// AIRLIFT PATCH (PyPortal-Pynt-RTK): BLE_NINA_SPI_TRANSPORT added as a
// build-flag opt-in. Upstream gates this transport on four Arduino board
// macros; Adafruit AirLift boards (pyportal_m4, metro_m4_airlift) speak the
// identical SPI protocol once nina-fw >= 3.0.0 is flashed, but define
// different board macros. Only change vs. ArduinoBLE 2.1.0 (tag 281377b),
// mirrored in local/BLELocalDevice.cpp.
#if defined(ARDUINO_AVR_UNO_WIFI_REV2) || defined(ARDUINO_SAMD_MKRWIFI1010) || defined(ARDUINO_SAMD_NANO_33_IOT) || defined(ARDUINO_NANO_RP2040_CONNECT) || defined(BLE_NINA_SPI_TRANSPORT)
